...
|---|------|------|------|------------|
| H1 | Push 26 commits to origin | 2 min | $0 | Nothing |
...
**Why:** 26 commits (BL-069 through BL-204) are sitting on local `main` but have never been pushed. CI hasn't run on them, GitHub Pages hasn't deployed, and the remote repo is 2+ weeks stale. Nothing the AI builds is "real" until this happens.

**AI already prepared:** All 26 commits are clean, tests pass locally (196 tests), `flutter analyze` is clean.
...
2. Verify what's pending:
   ...
   ```
   You should see 26 commits from `808abd0` (BL-069) to `96e79b8` (BL-204).
3. Push:
   ```bash
